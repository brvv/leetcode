class Solution:
    def minPathSum(self, grid: List[List[int]]) -> int:
        ...
        # two rows dp
        def minPathSumRow():
            row = [10000000000000 for item in grid[0]]
            
            for row_i, grid_row in enumerate(grid):
                new_row = [0 for item in grid[0]]
                for col_i, item in enumerate(grid_row):
                    cost = grid[row_i][col_i]
                    ...
                    if row_i == 0 and col_i == 0:
                        ...
                        top = row[col_i]
                        left = new_row[col_i - 1] if col_i - 1 >= 0 else 10000000000000
                        
                        new_row[col_i] = cost + min(top, left)
                row = new_row
                        
            return row[-1]
                    
        
        return minPathSumRow()