class Solution:
    def minimumTotal(self, triangle: List[List[int]]) -> int:
        ...
        def minimumTotalRow():
            ...
            for row_i, t_row in enumerate(triangle):
                
                curr_row = [item for item in triangle[row_i]]
                for col_i, cost in enumerate(t_row):
                    if row_i == 0 and col_i == 0:
                        curr_row[col_i] = cost
                    else:
                        left_parent = row[col_i - 1] if col_i - 1 >= 0 else 1000000000000000000000
                        right_parent = row[col_i] if col_i < len(row) else 1000000000000000000000
                        curr_row[col_i] = cost + min(left_parent, right_parent)
                row = curr_row
            return min(row)
        
        return minimumTotalRow()